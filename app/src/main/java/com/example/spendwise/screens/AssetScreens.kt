    ...
    Column(Modifier.fillMaxSize().verticalScroll(androidx.compose.foundation.rememberScrollState()).imePadding().padding(20.dp), verticalArrangement = Arrangement.spacedBy(14.dp)) {
        ...
        Button(onClick = {
            ...
            val asset = AssetEntity(id = existingAsset?.id ?: 0, name = name.trim(), type = type, brand = brand.clean(), model = model.clean(), purchaseDateEpochDay = purchaseDate,
                ...
                mileageUpdatedAt = if (mileage.isNotBlank() && mileage.toLongOrNull() != existingAsset?.currentMileageKm) now else existingAsset?.mileageUpdatedAt)
...
fun AssetDetailScreen(onBack: () -> Unit, onEdit: (Long) -> Unit, onCommitment: (Long) -> Unit,
    ...
    LazyColumn(Modifier.fillMaxSize().padding(horizontal = 16.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
        ...
        item { DetailSection("Overview") { listOfNotNull(asset.brand, asset.model).takeIf { it.isNotEmpty() }?.let { Text(it.joinToString(" ")) }; state.identifiers.forEach { Text("${it.label ?: it.type.label()}: ${it.value}", maxLines = 1, overflow = TextOverflow.Ellipsis) }; asset.currentMileageKm?.let { Text("Mileage: $it km") }; if (asset.type == AssetType.VEHICLE) { asset.mileageUpdatedAt?.let { updated -> val days = ChronoUnit.DAYS.between(
            Instant.ofEpochMilli(updated).atZone(ZoneId.systemDefault()).toLocalDate(), LocalDate.now()); if (days >= 30) Text("Mileage last updated $days days ago", color = MaterialTheme.colorScheme.onSurfaceVariant) }; TextButton(onClick = { dialog = "mileage" }) { Text("Update mileage") } } } }
